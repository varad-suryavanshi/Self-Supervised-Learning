...
class SimCLR(object):
    ...
    def train(self, train_loader):
        ...
        logging.info(f"Training with cuda disabled: {self.args.disable_cuda}.")

        accum_steps = getattr(self.args, "accum_steps", 1)
        self.optimizer.zero_grad()

        for epoch_counter in range(self.args.epochs):
            for step_idx, (images, _) in enumerate(tqdm(train_loader)):
                ...
                if n_iter % self.args.log_every_n_steps == 0:
                    ...
                    # log original (un-divided) loss
                    self.writer.add_scalar(
                        "loss", loss.item() * accum_steps, global_step=n_iter
                    )
                    self.writer.add_scalar(
            ...
            if epoch_counter >= 10:
                self.scheduler.step()
            logging.debug(
                f"Epoch: {epoch_counter}\tLoss: {loss.item() * accum_steps}"
                f"\tTop1 accuracy: {top1[0]}"
            )
            # -------- NEW: periodic checkpointing --------
            if ckpt_freq > 0 and (epoch_counter + 1) % ckpt_freq == 0:
                ckpt_name = f"checkpoint_{epoch_counter+1:04d}.pth.tar"
                ckpt_path = os.path.join(self.writer.log_dir, ckpt_name)
                save_checkpoint(
                    {
                ...
                logging.info(
                    f"Saved checkpoint at epoch {epoch_counter+1} to {ckpt_path}"
                )
            # ---------------------------------------------
